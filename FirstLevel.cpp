		...
		for (enemies.it = enemies.getFirst(); enemies.it.getIt() != nullptr; enemies.it++) {
			...
			if (!p1->getInvulneravel())
				if (enemies.it.getIt()->getInfo()->getCollisor()->CheckCollision(p1->getCollisor(), direction, 1.0f)){
					...
					if (enemies.it.getIt()->getInfo()->getLife() == 0)
						enemies.remove();
				}
			if (p2 != nullptr) {
...
void FirstLevel::load_static()
{
	...
	platforms + chao;
	Warlock* wlk = new Warlock(sf::Vector2f(250, 200), sf::Vector2f(100, -300), sf::Vector2f(150, 0), sf::Vector2f(101, 131), sf::Vector2f(15, -20), 1000,&projectiles);
	wlk->setP1(p1);
	enemies + wlk;
}
...
void FirstLevel::update(float deltat)
{
	...
	for (projectiles.it = projectiles.getFirst(); projectiles.it.getIt(); projectiles.it++) {
